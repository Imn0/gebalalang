...
use crate::{
    ast::{Command, Location}, try_or_err, error::{DisplayMessage, Message, MessageSeverity}, program::Program
};

trait Validator {
    fn validate_pass(&self) -> Result<(), ()>;
}

impl Program {
    pub fn ast_validate(&mut self) -> Result<(), ()> {
        let validators = [RecursiveCallValidator { program: &self }];

        for validator in validators {
            try_or_err!(validator.validate_pass());
        }
...
struct RecursiveCallValidator<'a> {
    program: &'a Program,
}

impl<'a> Validator for RecursiveCallValidator<'a> {
    fn validate_pass(&self) -> Result<(), ()> {
...
impl<'a> RecursiveCallValidator<'a> {
