...
use crate::{
    ast::{
        Ast, Command, Condition, Declaration, Either, Expression, ForDirection, Identifier,
        Location, MainBlock, ProcArgument, Procedure, Value,
    },
    code_gen::ir::LabelIdx,
};

use super::{
    ir::IR,
    memory::{Memory, SymbolLocation},
    CodeGenerator, ConstInfo, ProcedureInfo, MIN_CONST_USAGE,
};
...
impl CodeGenerator {
    pub fn generate_asm(&mut self, ast: &Ast) {
        ...
        if ast.procedures.len() > 0 {
            for (name, proc) in &ast.procedures {
    ...
    fn allocate_declarations(&mut self, declarations: &Vec<Declaration>) {
        for dec in declarations {
            let _ = self
                .memory
                .allocate_declaration(dec, &self.current_scope)
                .unwrap();
        }
    }
    ...
    fn compile_command(&mut self, command: &Command) {
        match command {
            Command::Assignment {
                ...
                self.compile_expression(identifier, expression, location.clone());
            }
            Command::If {
                condition,
                then_commands,
            ...
            Command::NoOp => todo!(), // _ => unimplemented!(),
        }
    }
    ...
    fn compile_expression(
        ...
        match expression {
            ...
            Expression::Multiplication(..) | Expression::Modulo(..) | Expression::Division(..) => {
                let left;
                let right;
                let proc_name;
            ...
            Expression::TimesPowerTwo(value, _) => todo!(),
            Expression::HalfTimes(value, _) => todo!(),
        }
    }
        ...
        match &ident.index {
            ...
            Some(idx) => match idx {
                Either::Left(idx_name) => {
                    let idx_location = self
                        .memory
                        .get_name_location(idx_name, &self.current_scope, ident.location.clone())
                        .unwrap();
